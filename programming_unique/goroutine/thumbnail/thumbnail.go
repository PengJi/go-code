...
func Image(src image.Image) image.Image {
	// Compute thnumbnail size ,preserving aspect ratio
	...
	yscale := float64(xs) / float64(width)
	xscale := float64(ys) / float64(height)
...
// ImageStrem reads an image from r and writes a thumnail-size version of it to w.
...
// ImageFile2 reads an image from infile and writes a thumbnail-size version of it to outifle
func ImageFile2(outfile, infile string)(error error){
